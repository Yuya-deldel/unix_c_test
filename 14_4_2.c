...
int main(void) {
    ...
    if ((pid = fork()) < 0) {
        ...
        while (fgets(line, MAXLINE, stdin) != NULL) {   // stdin -> line (行ごと)
            n = strlen(line);
            if (write(fd1[1], line, n) != n) err_sys("write error to pipe");    // line => child
            
            // child -> line
            if ((n = read(fd2[0], line, MAXLINE)) < 0) err_sys("read error to pipe");
            if (n == 0) {
